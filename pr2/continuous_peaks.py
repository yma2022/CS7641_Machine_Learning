...
util.plot_problem(data_fitness, iterations, ylabel="Best Fitness", title="Continuous Peaks Optimal by Iterations: Average of 10", show=False)
util.plot_problem(data_time, iterations, ylabel="Process Time", title="Continuous Peaks Process Time: Average of 10", show=False)
util.plot_problem(fevals, iterations, ylabel="# of Evaluations", title="Continuous Peaks Function Evaluations: Average of 10", show=False)
...
for i, s in enumerate(problem_sizes):
    ...
    restarts = util.rhc_tuning(problem)
    print("RHC restarts for Continuous Peaks: ", restarts)

    decay_param = util.sa_tuning(problem)
    print("SA decay param for Continuous Peaks: ", decay_param)
    # Define decay schedule
    schedule = mlrose.ExpDecay(init_temp=decay_param[0], exp_const=decay_param[1], min_temp=decay_param[2])

    pop_ga, mutation_p = util.ga_tuning(problem)
    print("GA pop size and mutation prob for Continuous Peaks: ", pop_ga, mutation_p)

    pop_m, keep_p = util.mimic_tuning(problem)
    print("MIMIC pop size and keep pct for Continuous Peaks: ", pop_m, keep_p)
    for k in range(10):
...
util.plot_problem_size(data_size, [10, 20, 30, 40, 50], ylabel="Best Fitness", title="Continuous Peaks Optimal by Size: Average of 10", show=False)
