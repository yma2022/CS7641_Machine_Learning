...
def svm_plot_learning_curve(dataset, estimator, X_train, y_train, param_range, title="", kernel='rbf', ylim=(0.5, 1.01), show=False):
    plt.figure()
    plt.title(title)
    plt.xlabel("Training examples")
    plt.ylabel("Score")
    ...
    if show:
        ...
        outfile = "img/"+ dataset + '_'  + title + '_' + kernel + ".png"
        plt.savefig(outfile)
        plt.close()

def svm_plot_validation_curve(dataset, clf_dt, X_train, y_train, title, xlabel, kernel='rbf', p_name="max_depth", p_range=np.arange(1, 31), cv=4, log=False, ylim=(0.5, 1.01), show=False):
    plt.figure()
    train_scores, valid_scores = validation_curve(clf_dt, X_train, y_train, param_name=p_name, param_range=p_range, cv=cv, n_jobs=-1, verbose=0)
    train_scores_mean = np.mean(train_scores, axis=1)
    train_scores_std = np.std(train_scores, axis=1)
