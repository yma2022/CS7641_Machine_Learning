...
class KNeareatNeighbor(object):
    ...
    def train(self, X_train, y_train):
        ...
        if self.verbose:
            util.plot_learning_curve(self.dataset, estimator=clf_knn, X_train=X_train, y_train=y_train, param_range=np.linspace(0.1,1,5), title="Learning Curve for KNN")
            util.plot_validation_curve(self.dataset, clf_knn, X_train, y_train, title="Validation Curve for KNN over n_neighbors", xlabel='n_neighbors', p_name="n_neighbors", p_range=np.arange(1, 30), cv=4)
